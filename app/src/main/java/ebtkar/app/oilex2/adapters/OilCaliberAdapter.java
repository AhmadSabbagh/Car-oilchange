...
import ebtkar.app.oilex2.activities.SelectOilBrandsActivity;
...
public class OilCaliberAdapter extends BasicRecycleviewAdapter implements Filterable {
    ArrayList original;
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (charString.isEmpty()) {
                    ...
                    List<OilType> filteredList = new ArrayList<>();
                    for (Object row : original) {

                        // name match condition. this might differ depending on your requirement
                        // here we are looking for name or phone number match
                        if (((OilType)row).getName().toLowerCase().contains(charString.toLowerCase())
                                ) {
                            filteredList.add((OilType)row);
                        }
                    }

                    contactListFiltered = (ArrayList<OilType>) filteredList;
                }
    ...
    public OilCaliberAdapter(ArrayList cats, Activity c) {
        super(cats, c);
        this.original=cats;
    }
    ...
    public void onBindViewHolder(@NonNull MyView holder, int position) {
       final OilType p = (OilType)cats.get(position);
        ModifiedView v = (ModifiedView)holder;

        v.title.setText(p.getName());
        if(!p.getPicture().equals("")&&!p.getPicture().equals("-"))
            Glide.with(context).load(p.getPicture())
                    .into(v.pic);
        else{
            Glide.with(context).load(R.drawable.car_oil)
                    .into(v.pic);
        }
...
v.body.setOnClickListener(new View.OnClickListener() {
    ...
    public void onClick(View v) {
        Intent ii = new Intent(context, SelectOilBrandsActivity.class);
        ii.putExtra("car_id" ,context.getIntent().getStringExtra("car_id"));
        ii.putExtra("car_model_id" ,context.getIntent().getStringExtra("car_model_id"));
        ii.putExtra("calib_id" ,p.getId());
        ii.putExtra("oil_name" ,p.getName());
        ii.putExtra("oil_pic" ,p.getPicture());
